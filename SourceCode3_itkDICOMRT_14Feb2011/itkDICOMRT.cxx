/*=========================================================================
  Program: DICOM-RT Structure file converter
  Module: itkDICOMRT.cxx
  Author: Jason Dowling, with gdcm usage code from gdcmscene.cxx authored by Mathieu Malaterre 
  ...
  Requires:  
  1. GDCM 2-0 release 2.0 installed (download from http://sourceforge.net/apps/mediawiki/gdcm/index.php?title=GDCM_Release_2.0 )
  2. Recent ITK (eg. 3.14) compiled with CMake flag ITK_USE_SYSTEM_GDCM = ON and GDCM_DIR set to correct path
  3. For each struct file (usually RS_something.dcm you will need to generate a template volume from the image data 
     (nb I just convert the dcm files into a niftii image) - Nb template image is no longer required (JD June 2010 )

  Test data provided is a pelvic MRI with manual contours of bladder, prostate, rectum, etc.
  
  Update History:
  June 2010 Jason Dowling 
    1. Removed need for niifi file and corrected origin issue.  Now outputs a niftii image file in addition to the contours
    2. Now trims contour names to remove blank spaces
    3. Also included updates by Guillaume Cazoula to correct the following problem: sometimes  the tags "ROI Contour Sequence" and 
    the corresponding  "Structure Set ROI Sequence" may not appear in the same order in the Dicom RT Structure Set File, leading to 
    miss-correspondences  between the labels and the segmented structure.  To find the correspondance we should compare the tags :
   (3006, 0022) for the Structure Set ROI Sequence and (3000, 0084) for the ROI Contour Sequence in the DICOM file.
 
  February 2011 Jason Dowling
    1. Corrected off by one slice alignment problem in mergeImages().  Thanks to Andrew Godley and Oscar Acosta for reporting.  
...
typedef itk::OrientedImage< PixelType, 2 >   ImageSliceType;
...
void mergeImages(ImageSliceType::Pointer tempSlice, ImageType::Pointer finalImage, int iRequiredSlice )
{
  ...
  for (int i=0;i<iX;i++)
    for (int j=0;j<iY;j++)
    {
      ...
      if (pixelValue != 0)
        finalImage->SetPixel(pixelIndex, pixelValue  );
    }
  }
}
...
int main(int argc, char * argv[])
{
  ...
  if( argc < 3 )
  {
    ...
    std::cerr << "eg. " << argv[0] << " itkDICOMRT E027 E027/RS.M1356941_.dcm E027" << std::endl; 
    return 1;
  }
